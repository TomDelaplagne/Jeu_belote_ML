...
class BeloteGameEnv(gym.Env):
    ...
    def __init__(self, *players: list[Player], deck = None):
        ...
        # 4 token des cartes en main parmis 16 cartes ou 0 = 4:17
        # 12 token des cartes deja jouees ou 0 = 12:17

        # self.observation_space = spaces.MultiBinary([16, 17])
        self.observation_space = spaces.Box(low=0, high=16, shape=(16,), dtype=int)
    ...
    def step(self, action):
        ...
        hand_observation = self.transform_card_list_to_token_list(self.game.players[0].hand, 4)
        played_cards_obs = self.transform_card_list_to_token_list(self.game.played_cards, 12)
        observation = hand_observation + played_cards_obs

        if self.trick_played == 4:
            done = True
    ...
    def transform_card_to_token(self, card):
        ...
        return suits.index(card.suit) * 4 + card.rank
