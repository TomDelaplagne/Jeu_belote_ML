...
from src.python.game.game_trick_manager_class import GameTrickManager
from src.python.game.game_scorer_class import GameScorer
...
class Game:
    def __init__(self, *players: list[Player], deck_src=None):
        ...
        self._trick_manager: GameTrickManager = GameTrickManager(self.players)
        self._scorer: GameScorer = GameScorer(self.players)

    def play(self):
        self.initialize_game()
        self.play_tricks()
        points = self.totalize_points()
        return points

    def initialize_game(self):
        """Shuffles, deals cards, set partners and starts bidding."""
        GameInitializer(self.players).initialize()

    def play_tricks(self):
        """Play all tricks of the game."""
        self.trick_manager.play_all_tricks()

    def totalize_points(self):
        """Calculate the points scored by each team in the current hand."""
        return self.scorer.totalize_points()
...
#             card = player.play_card(f"{player} can play any card", self.current_trick, self.trump_suit)
#             if led_suit is None:
#                 led_suit = card.suit
#             trick_winner = player
